export type QaCaseStoredVersion = {
  id: string;
...
export type QaCaseFetchedVersion = {
  id: string;
  contentHash: string;
  eTag: string | null;
  lastModified: string | null;
...
export function detectQaCaseChange({
  current,
  next,
}: {
  current: QaCaseStoredVersion | null;
  next: QaCaseFetchedVersion;
}) {
  if (current === null) {

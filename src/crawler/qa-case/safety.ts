...
export type QaCaseCrawlerState = {
...
export class QaCaseSafety {
  constructor(private readonly statePath: string) {}

  async loadState(): Promise<QaCaseCrawlerState> {
    try {
      const raw = await readFile(this.statePath, "utf8");
      return JSON.parse(raw) as QaCaseCrawlerState;
    } catch {
      return {
  ...
  private async writeState(state: QaCaseCrawlerState) {

export type TsutatsuStoredVersion = {
  id: string;
...
export type TsutatsuFetchedVersion = {
  id: string;
  contentHash: string;
  eTag: string | null;
  lastModified: string | null;
...
export function detectTsutatsuChange({
  current,
  next,
}: {
  current: TsutatsuStoredVersion | null;
  next: TsutatsuFetchedVersion;
}) {
  if (current === null) {

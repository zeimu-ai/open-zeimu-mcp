...
export type TaxAnswerCrawlerState = {
...
export class TaxAnswerSafety {
  constructor(private readonly statePath: string) {}

  async loadState(): Promise<TaxAnswerCrawlerState> {
    try {
      const raw = await readFile(this.statePath, "utf8");
      return JSON.parse(raw) as TaxAnswerCrawlerState;
    } catch {
      return {
  ...
  private async writeState(state: TaxAnswerCrawlerState) {

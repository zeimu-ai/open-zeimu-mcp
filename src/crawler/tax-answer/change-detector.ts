export type TaxAnswerStoredVersion = {
  id: string;
...
export type TaxAnswerFetchedVersion = {
  id: string;
  contentHash: string;
  eTag: string | null;
  lastModified: string | null;
...
export function detectTaxAnswerChange({
  current,
  next,
}: {
  current: TaxAnswerStoredVersion | null;
  next: TaxAnswerFetchedVersion;
}) {
  if (current === null) {
